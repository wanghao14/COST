...
from dataset import CaptionDataset as RCDataset
import nntrainer.utils as utils
...
class Translator(object):
    ...
    def __init__(self, model: nn.Module, cfg, logger: Optional[logging.Logger] = None):
        self.model = model
        self.cfg = cfg
        self.logger = logger
    ...
    def translate_batch_beam(self, input_ids_list, video_features_list, input_masks_list, token_type_ids_list,
        ...
        for cur_input_masks in input_ids_list:
            assert torch.sum(cur_input_masks[:, self.cfg.data.max_v_len + 1:]) == 0, (
                "Initially, all text tokens should be masked.")

        config = rt_model.cfg
        with torch.no_grad():
            prev_ms = [None] * config.model.num_hidden_layers
            ...
            for idx in range(step_size):
                prev_ms, dec_res = beam_decoding_step(
                    ...
                    rt_model, config.data.max_v_len, config.data.max_t_len, beam_size)
    ...
    def translate_batch_greedy(self, input_ids_list, video_features_list, input_masks_list, token_type_ids_list,
                               detect_ids_list, detect_features_list, detect_masks_list, detect_token_type_ids_list,
                               action_ids_list, action_features_list, action_masks_list, action_token_type_ids_list,
                               rt_model):
        def greedy_decoding_step(prev_ms_, input_ids, video_features, input_masks, token_type_ids,
                                 detect_ids, detect_features, detect_masks, detect_token_type_ids,
            ...
            for dec_idx in range(max_v_len, max_v_len + max_t_len):
                ...
                copied_prev_ms = copy.deepcopy(prev_ms_)  # since the func is changing data inside
                _, _, pred_scores, _, _ = model.forward_step(
                    copied_prev_ms, input_ids, video_features, input_masks, token_type_ids,
                    detect_ids, detect_features, detect_masks, detect_token_type_ids,
                    action_ids, action_features, action_masks, action_token_type_ids)
            ...
            cur_ms, _, pred_scores, _, _ = model.forward_step(
                prev_ms_, input_ids, video_features, input_masks, token_type_ids,
                detect_ids, detect_features, detect_masks, detect_token_type_ids,
                action_ids, action_features, action_masks, action_token_type_ids)
        ...
        input_ids_list, input_masks_list = self.prepare_video_only_inputs(
            input_ids_list, input_masks_list, token_type_ids_list)
        detect_ids_list, detect_masks_list = self.prepare_video_only_inputs(
            detect_ids_list, detect_masks_list, detect_token_type_ids_list
        )
        action_ids_list, action_masks_list = self.prepare_video_only_inputs(
            action_ids_list, action_masks_list, action_token_type_ids_list
        )
        for cur_input_masks in input_ids_list:
            assert torch.sum(cur_input_masks[:, self.cfg.data.max_v_len + 1:]) == 0,\
                "Initially, all text tokens should be masked"
        for cur_input_masks_det in detect_ids_list:
            assert torch.sum(cur_input_masks_det[:, -self.cfg.data.max_t_len:]) == 0, \
                "Initially, all text tokens should be masked"
        for cur_input_masks_act in action_ids_list:
            assert torch.sum(cur_input_masks_act[:, -self.cfg.data.max_t_len:]) == 0, \
                "Initially, all text tokens should be masked"
        ...
        with torch.no_grad():
            ...
            for idx in range(step_size):
                prev_ms, dec_seq = greedy_decoding_step(
                    prev_ms, input_ids_list[idx], video_features_list[idx],
                    input_masks_list[idx], token_type_ids_list[idx],
                    detect_ids_list[idx], detect_features_list[idx],
                    detect_masks_list[idx], detect_token_type_ids_list[idx],
                    action_ids_list[idx], action_features_list[idx],
                    action_masks_list[idx], action_token_type_ids_list[idx],
                    rt_model, config.data.max_v_len, config.data.max_t_len)
    ...
    def translate_batch_single_sentence_greedy(self, input_ids, video_features, input_masks, token_type_ids,
                                               detect_ids, detect_features, detect_masks, detect_token_type_ids,
                                               detect_cates, action_ids, action_features, action_masks, action_token_type_ids,
        ...
        detect_ids, detect_masks = self.prepare_video_only_inputs(detect_ids, detect_masks, detect_token_type_ids)
        action_ids, action_masks = self.prepare_video_only_inputs(action_ids, action_masks, action_token_type_ids)
        assert torch.sum(input_masks[:, self.cfg.data.max_v_len + 1:]) == 0, "Initially, all text tokens should be masked"
        assert torch.sum(detect_masks[:, self.cfg.data.max_d_len + 1:]) == 0, "Initially, all text tokens should be masked"
        assert torch.sum(action_masks[:, self.cfg.data.max_a_len + 1:]) == 0, "Initially, all text tokens should be masked"
        config = model.cfg
        max_v_len = config.data.max_v_len
        max_t_len = config.data.max_t_len
        bsz = len(input_ids)
        next_symbols = torch.LongTensor([start_idx] * bsz)  # (N, )
        for dec_idx in range(max_v_len, max_v_len + max_t_len):
            input_ids[:, dec_idx] = next_symbols
            input_masks[:, dec_idx] = 1
            _, pred_scores, _, _ = model.forward(input_ids, video_features, input_masks, token_type_ids, None, detect_ids,
                                           detect_features, detect_masks, detect_token_type_ids, None, action_ids,
                                            action_features, action_masks, action_token_type_ids, None)
            # suppress unk token; (N, L, vocab_size)
            pred_scores[:, :, unk_idx] = -1e10
            # next_words = pred_scores.max(2)[1][:, dec_idx]
            next_words = pred_scores[:, dec_idx].max(1)[1]
        ...
        if use_beam:
            if recurrent:
                input_ids_list, video_features_list, input_masks_list, token_type_ids_list,\
                    detect_ids_list, detect_features_list, detect_masks_list, detect_token_type_ids_list= model_inputs
                return self.translate_batch_beam(
                    input_ids_list, video_features_list, input_masks_list, token_type_ids_list,
                    detect_ids_list, detect_features_list, detect_masks_list, detect_token_type_ids_list,
                    self.model, beam_size=self.cfg.beam.size, n_best=self.cfg.beam.n_best,
                    min_length=self.cfg.gene_bounds.min_sen_len, max_length=self.cfg.gene_bounds.max_sen_len - 2,
                    block_ngram_repeat=self.cfg.gene_bounds.block_ngram_repeat, exclusion_idxs=[],
                    length_penalty_name=self.cfg.gene_bounds.length_penalty_name, 
                    length_penalty_alpha=self.cfg.gene_bounds.length_penalty_alpha, use_cuda=self.cfg.exp.use_cuda)
            else:
            ...
            if recurrent:
                input_ids_list, video_features_list, input_masks_list, token_type_ids_list,\
                    detect_ids_list, detect_features_list, detect_masks_list, detect_token_type_ids_list, \
                    action_ids_list, action_features_list, action_masks_list, action_token_type_ids_list = model_inputs
                if xl:
                ...
                if untied or mtrans:
                    ...
                    input_ids_list, video_features_list, input_masks_list, token_type_ids_list, detect_ids_list,\
                        detect_features_list, detect_masks_list, detect_token_type_ids_list, detect_cates_list,\
                        action_ids_list, action_features_list, action_masks_list, action_token_type_ids_list = model_inputs
                    return self.translate_batch_single_sentence_greedy(
                        input_ids_list, video_features_list, input_masks_list, token_type_ids_list, detect_ids_list,
                        detect_features_list, detect_masks_list, detect_token_type_ids_list, detect_cates_list,
                        action_ids_list, action_features_list, action_masks_list, action_token_type_ids_list, self.model.module)
